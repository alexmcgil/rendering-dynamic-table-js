...
function render(data) {
    ...
    tableHeader.innerHTML = `<th>Имя</th><th>Фамилия</th><th>Описание</th><th>Цвет глаз</th>`
    ...
    let tableData = data
        .map(p =>
            `<tr id="${p.id}">${[p.name.firstName, p.name.lastName, (p.about.substring(0, geLengthOfString()) + "..."), p.eyeColor]
                ...
                .map((el, position) => `<td data="${position}">${el}</td>`).join('')}</tr>`).join('');
...
function edit(e) {
    ...
    // Создаём div для редактирования, в нём будет поле для ввода и две кнопки
